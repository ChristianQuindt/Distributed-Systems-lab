...
class Process:
    ...
    def __receive(self):
        ...
        self.logger.debug("{} says: inhalt _receive: {}".format(self.process_id, _receive))
        ...
        if _receive:
            ...
            for i in self.other_processes:
                self.logger.debug("{} says inhalt von I : {}".format(self.process_id ,i))
                self.channel.send_to({i}, self.process_id)
            processesList = []
            self.logger.debug("{} says: inhalt seiner other_processes: {}".format(self.process_id, self.other_processes))
            for i in range(0, len(self.other_processes)-1):

                receive = self.channel.receive_from(self.other_processes, 10)
                processesList.append(receive[1])
                self.logger.debug("{} says not Timedout processes: {}".format(self.process_id, processesList))
            self.other_processes = []
            for j in processesList:
              self.other_processes.append(j)

            processesList.append(self.process_id)
            for x in processesList:
                self.all_processes.append(x)

            self.logger.debug("{} ist glücklich. Neue other_processes = {}".format(self.process_id, self.other_processes))
