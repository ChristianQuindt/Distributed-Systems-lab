...
class DummyChordClient:
    ...
    def run(self):
        ...
        req = nodes[random.randint(0, len(nodes)-1)]
        key = random.randint(0, self.channel.MAXPROC)
        self.channel.send_to({str(req)}, (constChord.LOOKUP_REQ, key))
        response = self.channel.receive_from({str(req)})
        print("final result "+ str(response[1][1]))
